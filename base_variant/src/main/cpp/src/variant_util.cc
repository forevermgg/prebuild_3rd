#include "../include/variant_util.h"
...
#define FLEXBUFFER_BUILDER_STARTING_SIZE 1024
...
static bool VariantToJson(const Variant& variant, bool prettyPrint,
  ...
  switch (variant.type()) {
    ...
    case Variant::kTypeMutableBlob: {
      // BASE_LOG(ERROR) << ("Variants containing blobs are not supported.");
      return false;
    }
  }
  return true;
...
Variant FlexbufferToVariant(const flexbuffers::Reference& ref) {
  switch (ref.GetType()) {
    ...
    case flexbuffers::FBT_BLOB:
      // BASE_LOG(ERROR) << ("Flexbuffers containing blobs are not supported.");
      break;
  }
  return Variant::Null();
}
